module.exports = (lf, slf, project) => {
  ...
  const InfoBlock = (...args) => Box('about_page--info_block', ...args);
                ...
                Wrapper(
                  ...
                  Div(
                    ...
                    InfoBlock(
                      ...
                      br(),
                      KV(
                        Model.get().main.founder.key,
                        Model.get().main.founder.value,
                      ),
                      KV(
                        Model.get().main.legalAdress.key,
                        Model.get().main.legalAdress.value,
                      ),
                      KV(
                        Model.get().main.schedule.key,
                        $p(
                    ...
                    InfoBlock(
                      List(
                        Model.get().structural.map(({ key, value }) =>
                          KV(key, value, 'comma'),
                        ),
                      ),
                    ),
                    ...
                    InfoBlock(
                      List(
                        Model.get().documents.map(({ href: sHref, value }) =>
                          Link(value, sHref, '_blank'),
                        ),
                      ),
                    ),
                    H2('Образование'),
                    InfoBlock(
                      ...Model.get().education.map((elem) =>
                        $p(
                          H3(elem.title),
                          br(),
                          elem.content.length > 0
                            ? List(
                                elem.content.map(({ href: sHref, value }) =>
                                  Link(value, sHref, '_blank'),
                                ),
                              )
                            : '',
                        ...
                        Model.get().schools.map((elem) => {
                          ...
                          if (aElem.length === 1) {
                            ...
                            for (let i = 0; i < aElem.length; i++) {
                              ...
                              if (i < aElem.length - 1) {
                                render.push(
                                  Link('ПДД онлайн', '/pdd-onlajn', '_blank'),
                                );
                              }
                            }
                            return $p(...render);
                    ...
                    H2('Доступная среда'),
                    InfoBlock(Text(Model.get().description)),
                    H2('Международное сотрудничество'),
                    InfoBlock(Text(Model.get().international)),
                  ),
                ),
